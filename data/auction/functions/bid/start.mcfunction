...
#> プレイヤー通知
    # playsound
    execute as @a[tag=Auction.Participant] at @s run playsound block.note_block.pling master @s ~ ~ ~ 1 1

    # tellraw
    tellraw @a[tag=Auction.Participant] [{"translate":"kota-server.auction.message.prefix"},{"translate":"kota-server.auction.message.bid_start"}]
